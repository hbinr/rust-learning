...
正如变量默认不可变一样，引用指向的值默认也是不可变的。 我们可以通过 **可变引用** 来解决这个问题
...
> 注意:**引用的作用域** `s` 从创建开始，一直持续到它**最后一次使用**的地方，这个跟变量的作用域有所不同，**变量的作用域**从创建持续到某一个花括号 `}`
...
悬垂引用也叫做悬垂指针，意思为**指针指向某个值后，这个值被释放掉了，而指针仍然存在，其指向的内存可能不存在任何值或已被其它变量重新使用**。
